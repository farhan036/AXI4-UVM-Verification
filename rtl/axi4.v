module axi4 #(
    ...
    reg [ADDR_WIDTH-1:0] write_addr_F, read_addr_F;
    reg [7:0] write_burst_len, read_burst_len;
    reg [7:0] write_burst_cnt, read_burst_cnt;
    reg [2:0] write_size, read_size;
    
    wire [2:0] write_addr_incr,read_addr_incr;  //   Edit from wire [ADDR_WIDTH-1:0] write_addr_incr,read_addr_incr; to [2:0]

    
    
    // Address increment calculation
    assign  write_addr_incr = (1 << write_size); // 4 >> 100   so it toggle from 000 to 100
    assign  read_addr_incr  = (1 << read_size);  // 4 >> 100   so it toggle from 000 to 100
    
    // Address boundary check (4KB boundary = 12 bits)
    assign write_boundary_cross = ((write_addr_F & 12'hFFF) + ((write_burst_len +1 ) << write_size)) > 12'hFFF;     // EDITED 
    assign read_boundary_cross = ((read_addr_F & 12'hFFF) + ((read_burst_len + 1) << read_size)) > 12'hFFF;         // EDITED
    ...
    // FSM states
    reg [1:0] write_state; //Edit to two bits
    localparam W_IDLE = 3'd0, // 00
               W_ADDR = 3'd1, // 01
               W_DATA = 3'd2, // 10
               W_RESP = 3'd3; // 11

    reg [1:0] read_state; // Edit to two bits
    localparam R_IDLE = 3'd0,
               R_ADDR = 3'd1,
               R_WAIT = 3'd2, // Added bec mem is synch
               R_DATA = 3'd3;

    // Registered memory read data for timing
    //reg [DATA_WIDTH-1:0] mem_rdata_reg;  >> // we must remove we don't use 
    ...
    always @(posedge ACLK or negedge ARESETn) begin
        if (!ARESETn) begin
            ...
            write_addr_F <= {ADDR_WIDTH{1'b0}};
            read_addr <= {ADDR_WIDTH{1'b0}};
            read_addr_F <= {ADDR_WIDTH{1'b0}};
            ...
            // mem_rdata_reg <= {DATA_WIDTH{1'b0}}; // Removed
            ...
            case (read_state)
                ...
                R_DATA: begin
                    ...
                    if (read_addr_valid && !read_boundary_cross) begin
                        RDATA <= mem_rdata;
                        mem_en <= 1'b1;                                 // Added.
                        mem_addr <= (read_addr + read_addr_incr) >> 2;  // Added.
                        RRESP <= 2'b00;  // OKAY
                    end else begin
                        RDATA <= {DATA_WIDTH{1'b0}};
                        RRESP <= 2'b10;  // SLVERR
